<!DOCTYPE html>
<html>
<head>
    <!--Import Google Icon Font-->
    <link href="http://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">
    <!--Import materialize.css-->
    <link type="text/css" rel="stylesheet" href="css/materialize.min.css"  media="screen,projection"/>

    <!--Let browser know website is optimized for mobile-->
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
</head>


<body>

<nav class="nav-extended">
    <div class="nav-wrapper">
        <a href="#" class="brand-logo">NBA</a>
        <a href="#" data-activates="mobile-demo" class="button-collapse"><i class="material-icons">menu</i></a>
        <ul id="nav-mobile" class="right hide-on-med-and-down">
            <li><a href="sass.html">Sass</a></li>
            <li><a href="badges.html">Components</a></li>
            <li><a href="collapsible.html">JavaScript</a></li>
        </ul>
        <ul class="side-nav" id="mobile-demo">
            <li><a href="sass.html">Sass</a></li>
            <li><a href="badges.html">Components</a></li>
            <li><a href="collapsible.html">JavaScript</a></li>
        </ul>
    </div>
    <div class="nav-content">
        <ul class="tabs tabs-transparent">
            <li class="tab"><a class="active" href="#test1">PLAYERS</a></li>
            <li class="tab"><a class="active" href="#test2">TEAMS</a></li>
            <li class="tab"><a href="#test3">MATCHES</a></li>
            <li class="tab"><a href="#test4">SEARCH</a></li>
        </ul>
    </div>
</nav>




<div id="test1" class="col s12">
    <div class="row">
        <div class="col s12 offset-s2">
            <div class="row">
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Name</th>
                            <th>Points</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Name</th>
                            <th>Rebounds</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Name</th>
                            <th>Assists</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
            </div>
            <div class="row">
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Name</th>
                            <th>Blocks</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Name</th>
                            <th>Steals</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Name</th>
                            <th>Turnovers</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
            </div>
            <div class="row">
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Name</th>
                            <th>Three Pointers Made</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Name</th>
                            <th>Free Throws Made</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Name</th>
                            <th>Fantasy Points</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>

</div>

<div id="test2" class="col s12">
    <div class="row">
        <div class="col s12 offset-s2">
            <div class="row">
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Team Name</th>
                            <th>Points</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Team Name</th>
                            <th>Rebounds</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Team Name</th>
                            <th>Assists</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
            </div>
            <div class="row">
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Team Name</th>
                            <th>Blocks</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Team Name</th>
                            <th>Steals</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Team Name</th>
                            <th>Field Goal Percentage</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
            </div>
            <div class="row">
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Team Name</th>
                            <th>Three Pointers Made</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Team Name</th>
                            <th>Three Point Percentage</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
                <div class="col s3">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th>Team Name</th>
                            <th>Fantasy Points Per Game</th>
                        </tr>
                        </thead>

                        <tbody>
                        <tr>
                            <td>Alvin</td>
                            <td>$0.87</td>
                        </tr>
                        <tr>
                            <td>Alan</td>
                            <td>$3.76</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        <tr>
                            <td>Jonathan</td>
                            <td>$7.00</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
</div>
<div id="test3" class="col s12">
    <div class="row">
        <div class="col s9 offset-s2">
            <div class="row">
                <div class="col s9">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th></th>
                            <th>1</th>
                            <th>2</th>
                            <th>3</th>
                            <th>4</th>
                            <th></th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr>
                            <td>team1</td>
                            <td>Alvin</td>
                            <td>$0.87</td>
                            <td>Alvin</td>
                            <td>$0.87</td>
                            <td>pts1</td>
                        </tr>
                        <tr>
                            <td>team2</td>
                            <td>Alan</td>
                            <td>$3.76</td>
                            <td>Alan</td>
                            <td>$3.76</td>
                            <td>pts2</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
            </div>
            <div class="row">
                <div class="col s9">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th></th>
                            <th>1</th>
                            <th>2</th>
                            <th>3</th>
                            <th>4</th>
                            <th></th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr>
                            <td>team1</td>
                            <td>Alvin</td>
                            <td>$0.87</td>
                            <td>Alvin</td>
                            <td>$0.87</td>
                            <td>pts1</td>
                        </tr>
                        <tr>
                            <td>team2</td>
                            <td>Alan</td>
                            <td>$3.76</td>
                            <td>Alan</td>
                            <td>$3.76</td>
                            <td>pts2</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
            </div>
            <div class="row">
                <div class="col s9">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th></th>
                            <th>1</th>
                            <th>2</th>
                            <th>3</th>
                            <th>4</th>
                            <th></th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr>
                            <td>team1</td>
                            <td>Alvin</td>
                            <td>$0.87</td>
                            <td>Alvin</td>
                            <td>$0.87</td>
                            <td>pts1</td>
                        </tr>
                        <tr>
                            <td>team2</td>
                            <td>Alan</td>
                            <td>$3.76</td>
                            <td>Alan</td>
                            <td>$3.76</td>
                            <td>pts2</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
            </div>
            <div class="row">
                <div class="col s9">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th></th>
                            <th>1</th>
                            <th>2</th>
                            <th>3</th>
                            <th>4</th>
                            <th></th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr>
                            <td>team1</td>
                            <td>Alvin</td>
                            <td>$0.87</td>
                            <td>Alvin</td>
                            <td>$0.87</td>
                            <td>pts1</td>
                        </tr>
                        <tr>
                            <td>team2</td>
                            <td>Alan</td>
                            <td>$3.76</td>
                            <td>Alan</td>
                            <td>$3.76</td>
                            <td>pts2</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
            </div>
            <div class="row">
                <div class="col s9">
                    <table class="striped">
                        <thead>
                        <tr>
                            <th></th>
                            <th>1</th>
                            <th>2</th>
                            <th>3</th>
                            <th>4</th>
                            <th></th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr>
                            <td>team1</td>
                            <td>Alvin</td>
                            <td>$0.87</td>
                            <td>Alvin</td>
                            <td>$0.87</td>
                            <td>pts1</td>
                        </tr>
                        <tr>
                            <td>team2</td>
                            <td>Alan</td>
                            <td>$3.76</td>
                            <td>Alan</td>
                            <td>$3.76</td>
                            <td>pts2</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
</div>
<div id="test4" class="col s12">
    <br><br>
    <div class="row">
        <form class="col s10 offset-s1" method="post"  action="servlet/SearchServlet">
            <div class="row">
                <div class="input-field col s2">
                    <input id="teamName1" type="text" class="validate">
                    <label for="last_name0">team1</label>
                </div>
                <div class="input-field col s2">
                    <input id="teamName2" type="text" class="validate">
                    <label for="last_name1">team2</label>
                </div>
                <div class="input-field col s2">
                    <input id="dateStart" type="text" class="validate">
                    <label for="last_name2">begin date</label>
                </div>
                <div class="input-field col s2">
                    <input id="dateEnd" type="text" class="validate">
                    <label for="last_name3">end date</label>
                </div>

                <button class="btn waves-effect waves-light col s1"
                        type="submit" name="submit" id="submitbutton1">Submit
                    <i class="material-icons right">send</i>
                </button>

            </div>
        </form>
    </div>

    <div class="row">
        <form class="col s10 offset-s1">
            <div class="row">
                <div class="input-field col s8">
                    <input id="last_name0" type="text" class="validate">
                    <label for="last_name0">Enter a STATE and return top 3 schools with highest player efficiency</label>
                </div>

                <button class="btn waves-effect waves-light col s1"
                        type="submit" name="submit" id="hubsubmitbutton">Submit
                    <i class="material-icons right">send</i>
                </button>
            </div>
        </form>
    </div>

    <div class="row">
        <form class="col s10 offset-s1">
            <div class="row">
                <div class="input-field col s2">
                    <input id="last_name0" type="text" class="validate">
                    <label for="last_name0">First Name</label>
                </div>
                <div class="input-field col s2">
                    <input id="last_name1" type="text" class="validate">
                    <label for="last_name1">Last Name</label>
                </div>
                <div class="input-field col s2">
                    <input id="last_name2" type="text" class="validate">
                    <label for="last_name2">Last Name</label>
                </div>
                <div class="input-field col s2">
                    <input id="last_name3" type="text" class="validate">
                    <label for="last_name3">Last Name</label>
                </div>
                <button class="btn waves-effect waves-light col s2"
                        type="submit" name="submit" id="hubsubmitbutton">Submit
                    <i class="material-icons right">send</i>
                </button>
            </div>
        </form>
    </div>

    <div class="row">
        <form class="col s10 offset-s1">
            <div class="row">
                <div class="input-field col s2">
                    <input id="last_name0" type="text" class="validate">
                    <label for="last_name0">First Name</label>
                </div>
                <div class="input-field col s2">
                    <input id="last_name1" type="text" class="validate">
                    <label for="last_name1">Last Name</label>
                </div>
                <div class="input-field col s2">
                    <input id="last_name2" type="text" class="validate">
                    <label for="last_name2">Last Name</label>
                </div>
                <div class="input-field col s2">
                    <input id="last_name3" type="text" class="validate">
                    <label for="last_name3">Last Name</label>
                </div>
                <button class="btn waves-effect waves-light col s2"
                        type="submit" name="submit" id="hubsubmitbutton">Submit
                    <i class="material-icons right">send</i>
                </button>
            </div>
        </form>
    </div>
</div>




<!--Import jQuery before materialize.js-->
<script src="https://code.jquery.com/jquery-2.1.1.min.js"></script>
<script src="js/materialize.js"></script>

</body>
</html>